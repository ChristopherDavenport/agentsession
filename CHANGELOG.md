...
- RFC 0001 is revised to draft 0.2. The summary now defines a session
  ...
  added: `run` (source and end reason), `dispatch` (a call handed to
  ...
  to the format rather than to one harness; how an input arrived and
  who decided a call are a harness's own detail. `outcome` gains `pass`, an `eval` kind, an
  unbounded `score` and a `target` that is an entry ID by rule; `env`
  gains `workspace`, a kind and one reference; the header gains `spawned_by` and derived
